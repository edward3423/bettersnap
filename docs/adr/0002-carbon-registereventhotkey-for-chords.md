...
We chose it because it is the only mechanism that satisfies [0001](./0001-zero-tcc-permissions-for-the-core.md): matching happens inside WindowServer, so our process gets **zero wakeups** while the user types in other apps, and it needs no permission. The sole alternative, `CGEventTap`, requires an Input Monitoring prompt and wakes us on every keystroke system-wide.
